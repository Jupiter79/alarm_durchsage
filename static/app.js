...
function renderConfigEditor() {
    ...
    html += createInput('Web UI Port', 'ui.port', currentConfig.ui.port, 'number', 'Auf welchem Port soll diese Webseite laufen? (Standard ist 80 (erreichbar unter http://alarm-server.local). Beispiel: Wenn hier 5000 steht, erreichst du die Seite unter http://alarm-server.local:5000. ACHTUNG: Nach einer Änderung muss das Programm komplett neu gestartet werden!)');
    html += createInput('DNS Name (mDNS)', 'server.dns_name', currentConfig.server?.dns_name || 'alarm-server', 'text', 'Unter welchem Namen soll das System im Netzwerk erreichbar sein? ACHTUNG: Nach einer Änderung muss das Programm komplett neu gestartet werden!)');
